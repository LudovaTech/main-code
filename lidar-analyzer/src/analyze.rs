...
#[inline]
fn check_around(
    accumulator: &mut [[u16; ANGLE_DISTRIBUTION]; DISTANCE_DISTRIBUTION],
    distance_case: usize,
    angle_case: usize,
    dist: usize,
    reward: u16,
...
fn build_hough_accumulator(points: &Vec<LidarPoint>) -> Vec<HoughLine> {
    ...
    let mut trie = Vec::with_capacity(ANGLE_DISTRIBUTION * DISTANCE_DISTRIBUTION);
    for (distance_case, angles) in accumulator.into_iter().enumerate() {
        for (angles_case, weight) in angles.into_iter().enumerate() {
            if weight < HOUGH_TRANSFORM_MIN_POINT_PER_LINE {
                continue;
            }
            trie.push(HoughLine {
                line: PolarLine {
                    distance: Meters(distance_case as f64 * DISTANCE_RESOLUTION.0),
                    angle: Rad::new(angles_case as f64 * angle_resolution.val()),
                },
                weight,
            })
        }
    ...
    fn load_log(from: &str) -> Box<Vec<LidarPoint>> {
        ...
        for point in from.split(';') {
            ...
            vector.push(LidarPoint {
                distance: Meters::mm(distance.into()),
